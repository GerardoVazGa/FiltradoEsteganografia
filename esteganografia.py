...
def abrirArch():
    ...
    ruta = filedialog.askopenfilename(
        ...
        filetypes=[("Todos los archivos", "*.*"), ("Archivos de imagen", "*.jpeg *.jpg *.png")]
    )

    extension = os.path.splitext(os.path.basename(ruta))[1].lower()
    print(extension)

    if extension in ('.jpeg', '.png', '.jpg'):
        messagebox.showinfo("Archivo Seleccionado", f"El archivo seleccionado: {ruta}")
        btnVerificarArchVentana.config(state=tk.NORMAL)
        btnEliminacion.config(state=tk.NORMAL)
    else:
        messagebox.showerror("Error", "No se seleccionó un archivo válido")
        btnVerificarArchVentana.config(state=tk.DISABLED)
        btnEliminacion.config(state=tk.DISABLED)
...
#[3]
btnVerificarArchVentana = tk.Button(window, text="Verificar archivo", command=abrirVentanaVerif, state=tk.DISABLED)
btnVerificarArchVentana.pack(padx=40, pady=10)

#[4]
btnEliminacion = tk.Button(window, text="Eliminar datos", command=abrirEliminacion, state=tk.DISABLED)
